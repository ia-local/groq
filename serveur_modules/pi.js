// index.js (VERSION FONCTIONNELLE ET DIAGNOSTIQUE POUR LES DEUX BOTS)
...
const botConfigs = {
   ...
   // },
    ...
    // 'neofs_Pibot': { // <-- Utilisez 'neofs_Pibot' si c'est le vrai username, sinon changez-le
    //     token: process.env.BOT_TOKEN_NEOFS_PIBOT,
    //     handler: require('./bots/neofsPibotHandler'),
    //     groqInstance: groq, // N'oubliez pas de passer Groq ici aussi si activé
    // },
    // 4. Pi-IA Bot (TOUJOURS COMMENTÉ POUR LE MOMENT)
    // 'Pi_ia_bot': { // <-- Utilisez 'Pi_ia_bot' si c'est le vrai username, sinon changez-le
    //     token: process.env.BOT_TOKEN_PI_IA_BOT,
    //     handler: require('./bots/piIaBotHandler'),
    //     openaiInstance: openai,
    // },
    // --- Laissez les autres bots commentés pour le moment ---
};

// --- Réseau de Coordination Multi-Bot ---
const botsNetwork = {
    workerPibot: { // Note: La clé ici doit correspondre à comment elle est appelée, pas nécessairement l'username
        processBackend: async (task) => {
            console.log("Simulating backend task processing in @worker_Pibot:", task);
            if (bots['worker_Pibot']) { // Vérifiez l'instance réelle du bot par son username
    ...
    neofsPibot: { // Assurez-vous que cette clé correspond aux appels (ex: botsNetwork.neofsPibot)
    ...
    piIaBot: { // Assurez-vous que cette clé correspond aux appels
...
async function setupBots() {
  ...
  //  const activeBotUsernames = ['Pi_bot', 'worker_Pibot']; // Active les deux bots pour le test
const activeBotUsernames = ['Pi_bot'];
